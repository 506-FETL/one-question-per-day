...
<Badge variant="secondary">封装</Badge>
...
<Code>
  ...
  export default function App() {
    return <Button variant="secondary" size="icon" className="size-8">
      <ChevronRightIcon />
    </Button>
  }
  ```
...
<Code>
  ...
  export default function App() {
    return <Button size="sm" disabled leftIcon={<Loader2Icon className="animate-spin" />}>
      Please wait
    </Button>
  }
  ```
