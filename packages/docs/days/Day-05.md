...
describe('deepClone function', () => {
  ...
  it('应在处理循环引用时正常运行', () => {
    const obj: Record<string, string | object> = { name: 'Alice' }
    obj.self = obj // 循环引用
    const clone = deepClone(obj)
    expect(clone).not.toBe(obj)
    expect(clone.name).toBe('Alice')
  ...
  it('应在对象包含方法时正常运行，并保持方法的功能以及方法的属性', () => {
    ...
    (objWithMethod.getValue as any).a = objWithMethod
    const clonedObjWithMethod = deepClone(objWithMethod)

    // 检查克隆后的对象结构
    expect(clonedObjWithMethod).not.toBe(objWithMethod)
    expect((clonedObjWithMethod.getValue as any).a.value).toBe(42)
...
| 类型    | 路径                                                                                                                                    |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| JS 版本 | [problems/Day 05/answer.js](https://github.com/506-FETL/one-question-per-day/blob/main/packages/problems/base/Day%2005/answer.js)       |
| TS 版本 | [problems/Day 05/ts/answer.ts](https://github.com/506-FETL/one-question-per-day/blob/main/packages/problems/base/Day%2005/ts/answer.ts) |
