...
1. **循环遍历**：使用 while 循环替代递归，避免栈溢出
2. **三重判断**：依次检查子节点、兄弟节点、父节点
3. **回溯机制**：无子节点和兄弟节点时向上回溯
4. **边界处理**：到达根节点时终止遍历
...
2. 处理 B，进入子节点 D
3. 处理 D，无子节点，处理兄弟 E
4. 处理 E，无子节点和兄弟，回溯到 B
5. B 的兄弟是 C，处理 C
6. C 无子节点和兄弟，回溯到 A（根节点），结束
```

### 时间复杂度

- **O(n)**：每个节点访问一次，n 为节点总数

### 空间复杂度

- **O(1)**：只使用常数级别的额外空间，无递归调用栈
...
1. **Fiber 三指针**：child（子）、sibling（兄弟）、return（父）
2. **遍历优先级**：子节点 > 兄弟节点 > 父节点回溯
3. **循环遍历**：用循环替代递归，空间复杂度 O(1)
4. **边界检查**：根节点作为遍历终止条件
5. **回溯逻辑**：无兄弟节点时向上回溯直到找到兄弟或根节点
6. **深度优先**：每个节点都会被访问且只访问一次

## 🤔 扩展思考

### 与传统树遍历的差异

- **传统递归**：使用函数调用栈，空间复杂度 O(h)
- **Fiber 循环**：使用指针遍历，空间复杂度 O(1)
- **可中断性**：Fiber 遍历可以随时暂停和恢复
