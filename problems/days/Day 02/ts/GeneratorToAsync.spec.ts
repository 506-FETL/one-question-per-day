...
describe('04.17--default.将生成器函数转换成异步函数', () => {
  it('应将生成器函数转换为异步函数', async () => {
    ...
    const asyncFunction: AsyncFunction<number> = generatorToAsync(generatorFunction)
  ...
  it('如果输入不是一个生成器函数,应抛出 TypeError', () => {
    const invalidFunc = () => {}
    expect(() => generatorToAsync(invalidFunc as unknown as GeneratorFunction)).toThrow(
      TypeError,
    )
    expect(() => generatorToAsync(null as unknown as GeneratorFunction)).toThrow(
      TypeError,
    )
    expect(() => generatorToAsync(123 as unknown as GeneratorFunction)).toThrow(TypeError)
  })
  ...
  it('如果返回的Promise被reject,应返回reject的报错值', async () => {
    ...
    const asyncFunction: AsyncFunction<void> = generatorToAsync(generatorFunction)
