...
**建议不要只单独运行测试脚本，最好直接运行 `pnpm dev`，这个会在运行测试之前进行检查和格式化，最后才会测试**
...
4. 本项目只会运行 src 文件夹下的测试，不会运行 `__SPEC__` 文件下的测试，目的是为了在给出题目之后，就算没有给出答案依然可以上传到远端，**但必须在未给出标答的题目文件中标注：// TODO**
